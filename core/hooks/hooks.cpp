...
void Hooks::init( ) {
	...
	m_client.add( 22, util::force_cast( &Hooks::CreateMove_Proxy ) );
	...
	m_player.add( Player::WEAPONSHOOTPOSITION, util::force_cast( &Hooks::Weapon_ShootPosition ) ); // why not working ?
	...
	m_game_movement.m_vtable = reinterpret_cast<uintptr_t**>(g_csgo.m_game_movement);
	m_game_movement.Init();
	m_game_movement.add(1, util::force_cast(&Hooks::ProcessMovement));

	m_key_values_system.m_vtable = reinterpret_cast<uintptr_t**>(g_csgo.m_key_values_system);
	m_key_values_system.Init();
	m_key_values_system.add(2, util::force_cast(&Hooks::AllocKeyValuesMemory));
