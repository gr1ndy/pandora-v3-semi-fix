...
bool SetupPacket(int sequence_number, bool* pbSendPacket)
{
	...
	g_cl.m_packet = pbSendPacket;
	return true;
}

bool Hooks::CreateMove(int sequence_number, float input_sample_frametime, bool active, bool& bSendPacket)
{
	Stack   stack;
	bool    ret;

	// let original run first.
	ret = g_hooks.m_client.GetOldMethod< CreateMove_t >( 22 )( this, sequence_number, input_sample_frametime, active);
