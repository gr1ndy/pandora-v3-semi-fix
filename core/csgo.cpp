...
bool CSGO::init( ) 
{
	...
	m_game_movement = interfaces.get< CGameMovement* >(HASH("GameMovement"));
	m_key_values_system = PE::GetExport(m_vstdlib_dll, HASH("KeyValuesSystem")).as< KeyValuesSystemFn >()();
	...
	m_globals = **reinterpret_cast<CGlobalVarsBase***>((*reinterpret_cast<std::uintptr_t**>(m_client))[11u] + 0xau);
	m_client_mode = **reinterpret_cast<IClientMode***>((*reinterpret_cast<std::uintptr_t**>(m_client))[10u] + 0x5);
	m_cl = **reinterpret_cast<CClientState***>((*reinterpret_cast<std::uintptr_t**>(m_engine))[12u] + 0x10);
	...
	m_hud                = pattern::find( m_client_dll, XOR( "B9 ? ? ? ? 68 ? ? ? ? E8 ? ? ? ? 89" ) ).add( 0x1 ).get( ).as< CHud* >( );//
	m_gamerules = pattern::find( m_client_dll, XOR( "A1 ? ? ? ? 85 C0 0F 84 ? ? ? ? 80 B8 ? ? ? ? ? 74 7A" ) ).add( 0x1 ).get< C_CSGameRules* >( );
	m_beams = pattern::find( m_client_dll, XOR( "B9 ? ? ? ? A1 ? ? ? ? FF 10 A1 ? ? ? ? B9" ) ).add( 0x1 ).get< IViewRenderBeams* >( );//
	...
	m_input = *pattern::find(m_client_dll, XOR("B9 ? ? ? ? FF 75 08 FF 50 0C")).add(0x1).as< CInput** >();
	...
	CL_Move = pattern::find(m_engine_dll, XOR("55 8B EC 81 EC ? ? ? ? 53 56 8A F9")).as<uintptr_t>();
