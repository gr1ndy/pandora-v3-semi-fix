...
public:
	// virtual indices
	enum indices : size_t {
		WORLDSPACECENTER = 78 + 1,
		GETMAXHEALTH = 122 + 1,
		ISPLAYER = 158,
		ISBASECOMBATWEAPON = 166 + 1,
	};
...
public:
	enum indices : size_t {
		...
		TESTHITBOXES = 52,
		THINK = 138 + 1,
		BUILDTRANSFORMATIONS = 189 + 1,
		DOEXTRABONEPROCESSING = 197 + 1,
		STANDARDBLENDINGRULES = 205 + 1,
		UPDATECLIENTSIDEANIMATION = 223 + 1, // 55 8B EC 51 56 8B F1 80 BE ? ? ? ? ? 74 36
		GETACTIVEWEAPON = 267 + 1,
		WEAPONSHOOTPOSITION = 282 + 1,
		CALCVIEW = 276 + 1,
		PRETHINK = 317 + 1,
		RUNPOSTTHINK = 319 + 1,
		GETFOV = 326 + 1,
		UPDATECOLLISIONBOUNDS = 329 + 1 // 56 57 8B F9 8B 0D ? ? ? ? F6 87 ? ? ? ? ?
	};
...
public:
	...
	__forceinline void EyePosition( vec3_t* pos ) {
		util::get_method< float* ( __thiscall* )( Player*, vec3_t* ) >( this, 168 + 1)( this, pos );
	}
	...
	__forceinline vec3_t Weapon_ShootPosition( ) {
		...
		if( *reinterpret_cast < int32_t* > ( uintptr_t( this ) + 0x3AB4 + 0x4 ) ) {
	...
	__forceinline bool alive( ) {
		...
		//return util::get_method< bool(__thiscall*)(decltype(this)) >(this, 155 + 1)(this);

		return m_lifeState() == LIFE_ALIVE && m_iHealth();
...
public:
	enum indices : size_t {
		...
		GETMAXCLIP1 = 367 + 1,
		GETSPREAD = 452 + 1,
		GETWPNDATA = 460 + 1, // C_WeaponCSBaseGun::GetCSWpnData
		GETINACCURACY = 482 + 1,
		UPDATEACCURACYPENALTY = 483 + 1,
	};
