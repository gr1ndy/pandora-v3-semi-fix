...
void events::round_end( IGameEvent* evt ) {
	...
	int reason = evt->GetInt( XOR( "reason" ) );
...
void events::player_hurt( IGameEvent* evt ) {
    ...
    if( evt && g_cfg[XOR("misc_logs_harmed")].get<bool>() ) {
        attacker = g_csgo.m_entlist->GetClientEntity<Player*>(g_csgo.m_engine->GetPlayerForUserID( evt->GetInt( XOR( "attacker" ) ) ) );
        victim   = g_csgo.m_entlist->GetClientEntity<Player*>(g_csgo.m_engine->GetPlayerForUserID( evt->GetInt( XOR( "userid" ) ) ) );
		...
		if (attacker != g_cl.m_local && victim == g_cl.m_local) {
			...
			g_notify.add(tfm::format(XOR("harmed by %s in the %s for %i damage."), name, g_shots.m_groups[evt->GetInt(XOR("hitgroup"))], evt->GetInt(XOR("dmg_health"))));
		}
...
void events::bullet_impact( IGameEvent* evt ) {
	...
	attacker = g_csgo.m_engine->GetPlayerForUserID(evt->GetInt(XOR("userid")));
...
void events::player_death( IGameEvent* evt ) {
	...
	int index = g_csgo.m_engine->GetPlayerForUserID( evt->GetInt( XOR( "userid" ) ) );
...
void events::bomb_planted( IGameEvent* evt ) {
    ...
    // get the func_bomb_target entity and store info about it.
     bomb_target = g_csgo.m_entlist->GetClientEntity( evt->GetInt( XOR( "site" ) ) );
    if( bomb_target ) {
	...
	player_index = g_csgo.m_engine->GetPlayerForUserID(evt->GetInt(XOR("userid")));
...
void events::bomb_begindefuse( IGameEvent* evt ) {
	...
	bool kit = evt->GetBool( XOR( "haskit" ) );
