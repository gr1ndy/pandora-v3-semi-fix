...
void Shots::OnHurt( IGameEvent* evt ) {
	...
	attacker = g_csgo.m_engine->GetPlayerForUserID( evt->GetInt( XOR( "attacker" ) ) );
	victim = g_csgo.m_engine->GetPlayerForUserID( evt->GetInt( XOR( "userid" ) ) );
	...
	damage = ( float )evt->GetInt( XOR( "dmg_health" ) );

	// get remaining hp.
	hp = evt->GetInt( XOR( "health" ) );
		...
		if( it->m_impacted && !it->m_hurt && it->m_confirmed ) {
			...
			if( g_cfg[ XOR( "cheat_mode" ) ].get<int>( ) == 0 ) {
				if( !g_aimbot.CanHit( start, end, it->m_record, it->m_hitbox, true, it->m_matrix ) ) {
					g_notify.add( XOR( "[ pandora] shot missed due to spread" ) );
			...
			g_notify.add( XOR( "[ pandora] shot missed due to bad resolve" ) );
